...
```mermaid
flowchart LR
    T["Notice a trigger<br/>(need · priority · reference<br/>· reflection · intent)"] --> A["Act<br/>(once, or on a rhythm)"]
    A --> C["Curate<br/>(keep what's necessary,<br/>cut the rest, classify)"]
    C --> R["Reflect weekly<br/>(review, find the refinement)"]
    R --> T
```
