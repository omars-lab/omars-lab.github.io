...
```mermaid
flowchart LR
    A([Determine value:<br/>what's worth knowing?]) --> B[Measure<br/>set up the metric]
    B --> C[Track<br/>record over time]
    C --> D[Report<br/>dashboards & trends]
    D --> E[Evaluate<br/>what does it mean?]
    E -->|adjust what matters| A
    E -->|act differently| C
    F[(Tooling &<br/>catalog)] -.supports.- B
    F -.supports.- C
    F -.supports.- D
```
