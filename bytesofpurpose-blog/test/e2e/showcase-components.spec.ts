...
test.describe('Component showcases (/handbook/components/*)', () => {
  ...
  // The Handbook sidebar carries a doorway OUT to the Changelog (/changelog, its own instance).
  // It's a sidebar `link` item, last after the autogenerated reference docs.
  test('Handbook sidebar links out to the Changelog', async ({ page }) => {
    await loadShowcase(page, '/handbook/components/structural/card');
    const sidebar = page.locator('nav.menu, .theme-doc-sidebar-container').first();
    const changelogLink = sidebar.locator('a.menu__link[href="/changelog"]');
    await expect(changelogLink, 'Handbook sidebar should have a Changelog link → /changelog').toBeVisible({ timeout: 10000 });
    await expect(changelogLink).toContainText('Changelog');
  });
});
