...
graph LR
    ...
    style engine fill:#e8f0fe,stroke:#1a73e8
    style signals fill:#fef7e0,stroke:#f9ab00
    style people fill:#e6f4ea,stroke:#34a853
```
...
graph LR
    ...
    style OWNER fill:#e6f4ea,stroke:#34a853
    style OP fill:#fef7e0,stroke:#f9ab00
    style SHOPPER fill:#f5f5f5,stroke:#9e9e9e
    style OWNER_M fill:#ffffff,stroke:#34a853
    style OP_M fill:#ffffff,stroke:#f9ab00
    style SH_M fill:#ffffff,stroke:#9e9e9e
```
...
- **Time to first shipped win**: **[Assumption: ≤ 30 days]** from store onboarding to the first statistically-validated, shipped improvement.
- **Decision yield**: **[Assumption: ≥ 70%]** of launched experiments reach a *valid* decision (ship or kill on defensible data) rather than timing out inconclusive.
- **Net conversion lift per store**: **[Assumption: +10% cumulative conversion lift within 6 months]**, measured against a held-back baseline.
- **False-win rate**: **[Assumption: ≤ 5%]** of shipped winners later shown (via holdback / re-test) to be neutral or negative.
- **Operator leverage**: **[Assumption: 1 operator supervises ≥ 25 active stores]** at steady state, trending upward.
- **Revenue-safety**: **[Assumption: zero unrecovered revenue-harm incidents]**; any experiment causing a material conversion/revenue drop is auto-rolled-back within a bounded window. _(This is a hard guardrail, not a soft target.)_
...
graph LR
    ...
    style owned fill:#e8f0fe,stroke:#1a73e8
    style assumed fill:#f5f5f5,stroke:#9e9e9e
```
...
graph LR
    ...
    style NOTHING fill:#f5f5f5,stroke:#9e9e9e
    style GUESS fill:#fce8e6,stroke:#d93025
    style ABANDON fill:#fce8e6,stroke:#d93025
```
...
**[Assumption:** target stores are predominantly Shopify or Shopify-like; the design should not hard-assume Shopify but Shopify is the dominant case to optimize for.**]** This shapes the execution-model options in [§6](#6-proposed-design).
...
graph LR
    ...
    style brain fill:#e8f0fe,stroke:#1a73e8
...
graph LR
    ...
    AGENT --> LEDGER[(Ledger)]
    style AGENT fill:#e8f0fe,stroke:#1a73e8
    style EDGE fill:#e6f4ea,stroke:#34a853
```
...
graph LR
    ...
    AGENT --> LEDGER[(Ledger)]
    style AGENT fill:#e8f0fe,stroke:#1a73e8
    style SHOPIFY fill:#fff0f0,stroke:#d93025
```
...
graph LR
    ...
    style brain fill:#e8f0fe,stroke:#1a73e8
    style C6 fill:#fef7e0,stroke:#f9ab00
    style C7 fill:#e6f4ea,stroke:#34a853
```
...
graph LR
    ...
    style OP fill:#fef7e0,stroke:#f9ab00
    style AGENT fill:#e8f0fe,stroke:#1a73e8
    style SCANNER fill:#f5f5f5,stroke:#9e9e9e
    style SHOPPER fill:#f5f5f5,stroke:#9e9e9e
```
...
- **No measurable storefront slowdown.** The variant-serving layer must add **[Assumption: &lt; 50ms]** to page load and **must not cause flicker (FOOC)**: the shopper never sees control flash before treatment. Native Shopify rendering (Phase A) and anti-flicker on the edge worker (Phase B) are the mechanisms.
- **Real-time health monitoring latency.** Guardrail health checks (C6) evaluate store health on a **[Assumption: ≤ 1-minute]** cadence so harm is caught and rolled back fast.
- **Decision freshness.** The measurement engine (C4) recomputes an experiment's decision at least **[Assumption: hourly]**, more often for high-traffic stores.
...
- **Many stores per operator.** The system must support the §2.3 target of **[Assumption: ≥ 25 active stores per operator]**, scaling to hundreds of concurrent stores without linear operator growth: the managed-service margin thesis.
...
- **Platform limits (Phase A):** Shopify app review, theme/app API rate limits, and what the platform permits an app to mutate constrain the action space. **[Assumption:** Shopify's app + theme APIs permit the copy/layout mutations the autonomous tier needs.**]**
