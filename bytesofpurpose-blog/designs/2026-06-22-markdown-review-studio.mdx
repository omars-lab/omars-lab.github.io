...
source:
  ...
  status: In Review
  imported: '2026-06-23'
mockups: ./_mockups/markdown-review-studio.mdx
diagrams:
  - type: context
    confidence: medium
    heading: 1.3 Context
  ...
  - type: flow
    confidence: medium
    heading: 4.1 How Document Review Works Today
  - type: flow
    confidence: medium
    heading: 6.1 Target State Overview
  - type: arch
    confidence: medium
    heading: 'Option 1: Claude Code CLI headless, auto-apply + commit _(recommended)_'
  - type: arch
    confidence: medium
    heading: 'Option 2: Anthropic API direct, server applies edits'
  - type: arch
    confidence: medium
    heading: 'Option 3: Export structured prompt for manual paste'
  - type: context
    confidence: medium
    heading: 'D10: Deployment topology (dual-mode) ⭐'
  - type: er
    confidence: high
  ...
  - type: usecase
    confidence: medium
    heading: 10.2 Use Case Diagram
  - type: sequence
    confidence: high
    heading: '10.3 Sequence Diagram: Full Review Round'
---
...
import Mockups from './_mockups/markdown-review-studio.mdx';
...
graph LR
  subgraph system[System]
    REV["👤 Reviewer (primary)"]
    REVuc(["click the exact spot, comment in place, no re-describing"])
    AUTH["✍️ Author"]
    AUTHuc(["edit markdown directly, keep source canonical"])
    CC["🤖 Claude Code"]
    CCuc(["structured anchored comments to act on, preserve markers"])
    GIT["🌿 Git"]
    GITuc(["clean, attributable commits = durable iteration history"])
    TEAM["🧑‍🤝‍🧑 Teammate / Stakeholder"]
    TEAMuc(["async review on any device, shared via the git host"])
  end
  REV --- REVuc
  AUTH --- AUTHuc
  CC --- CCuc
...
<Assumption>In the initial version, Reviewer and Author are the same person (Omar) on a single machine. Multi-user / concurrent review is a secondary, later concern: flagged as a key decision in §6.5.</Assumption>
...
<Assumption>The target folder is already a git repository. If not, the tool offers to `git init` it on first run; flagged as an open question in §10.3.</Assumption>
...
<Assumption>The author currently does not use GitHub PR review for these co-design docs: review is local and conversational with Claude. If PR-based review is in active use, §6 should consider integrating with it rather than replacing it.</Assumption>
...
<Assumption>A single doc has at most a few hundred comments and is well under the 10MB headless stdin cap; docs are referenced by path to Claude, not piped, so size is not a constraint.</Assumption>
