...
certificate. Most of that record is free and online. It is just scattered: a different
portal in every city, each with its own schema. And nobody wants to learn Socrata's query
dialect to check whether the vacant storefront on their block is quietly becoming a
restaurant.

Anyone watching a block can read that trail: a resident curious about the corner lot, a
would-be tenant scouting rents, a local reporter, a small-business owner sizing up the
competition. The record is public; it is just unreadable in practice. So I packaged the
knowledge (where to look, in what order, and how to read it) into a Claude Code plugin
called **`local-guide`**. You ask in plain English; the plugin drives a small
standard-library CLI over open data and reports the filings with citations. No API keys,
no accounts.
...
None were on a map yet. All were on the public record.

## Why a plugin and not just a script

A raw script needs you to already know which portal, which dataset ID, which columns, and
how to read a change-of-use. The skills encode all of that, so a plain-English question
resolves to the right calls:
