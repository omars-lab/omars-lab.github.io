...
2. **Dev pulls the key from the *real* Worker.** There is **no dev-only key and no
   ...
   Worker** (`https://blog.bytesofpurpose.com/api/*`), carrying the reader's real
   Cloudflare Access cookie (sign in once on the domain). So `localhost` unlocks premium
   through the exact same `/api/unlock-key` path as prod — the client code uses relative
   `/api/*` URLs unchanged, and the proxy bridges the origin. (This replaces the previous
   localhost "Sign in shows a toast" graceful-degrade — it's no longer needed, because
   `/api/*` now works in dev.)
...
and the dev server **proxies `/api/*` to the deployed Worker**, so signing in (once, on
the real domain) lets `localhost` unlock premium through the exact same path as prod.
There is no dev-only key and no "it no-ops locally" caveat. The `?internal=1` analytics
opt-in also works in dev. *(Parity requires the Worker to be deployed; until then
`/api/unlock-key` returns nothing in dev and prod alike — still consistent.)*
