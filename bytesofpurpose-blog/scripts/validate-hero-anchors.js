...
const ANCHORS = [
  ...
  {file: TSX, needle: /boardSweepProgress/, what: 'index.tsx: boardSweepProgress (StudioFacade quantizes boardProgress to the board column resolution and feeds SplitFlap SWEEP mode: the pickets board is SCRUBBED by the crossing phase)'},
  {file: TSX, needle: /function forcedProgress\b/, what: 'index.tsx: forcedProgress (?hero-progress=P test seam: pins raw progress, freezes a specific pickets crossing PHASE)'},
  {file: TSX, needle: /function useCatchUpProgress\b/, what: 'index.tsx: useCatchUpProgress (pickets display progress CHASES raw scroll: 70ms ease + speed cap, so flicks sweep through every picket instead of teleporting; GSAP numeric-scrub equivalent)'},
  ...
  {file: SF_TSX, needle: /sweepProgress/, what: 'SplitFlap/index.tsx: sweepProgress + sweepFromText (SWEEP mode: the board is scrubbed by scroll; a flip front splits destination letters (left) from previous text (right), only the crossed column flips)'},
  ...
  {file: CSS, needle: /\.studioPicket\b/, what: 'index.module.css: .studioPicket (one strip of the picket flash wave; +0.5px overlap kills the seam)'},
