...
durable roots plus the Handbook. **`craft`** (how I see the world — the professional topics:
generative-ai, software-development, product-management, productivity, blogging, interview-prep,
companies, entrepreneurship), **`journey`** (how I see myself — faith, personal-growth; the plugin
`id` is still `'self'` internally, but the folder/route are `journey`), **`knowledge`** (things I
know / how I understand things — the durable MENTAL MODELS, grouped ai/career/algorithms/
interviewing), **`habits`** (how I habitually operate — the practices, grouped professional/personal/
spiritual), and **`legend`** → `/handbook` (the reader's guide). Each is registered as its own
...
renaming / splitting an instance is owned by the `manage-docs-instances` skill** (the 7-touchpoint
...
| Manage docs instances | `manage-docs-instances` | the level ABOVE `reorganize-content`: adding / renaming / SPLITTING a whole root DOCS INSTANCE (a top-level `/craft`-style resource like `/knowledge` or `/habits`). Owns the **7-touchpoint registration checklist** (the `plugin-content-docs` block + the `docSidebar` navbar item, the `sidebars-<name>.js` file, the `docs/<name>/` dir, and the 4 parallel hand-maintained instance lists in `validate-redirects`/`validate-seo`/`generate-component-usage`/`generate-todos` — including the `generate-component-usage` ternary that SILENTLY misroutes a new instance to handbook, now a lookup) + the CROSS-INSTANCE content move (each doc is a `reorganize-content` move). Encodes the gotchas proven building `/knowledge` + `/habits`: **slug must be INSTANCE-RELATIVE** (`/ai/x`, not `/knowledge/ai/x` — the prefix DOUBLES), the **stale dev-server route table** (restart `:3000` after adding an instance or new pages/drafts won't show), **draft targets can't be redirected**, **pin the landing as `👋 Welcome`** (`sidebar_label` + `sidebar_position: 0`), and how to gather the instance's **visual identity** (navbar emoji/label + the arched `/img/cards/<name>.png` chooser card via `import-arched-image` + `maintain-homepage-hero`). Triggers on "make X its own root / its own resource / split X out into /X / add a docs section / rename the /Y route". Pairs with `reorganize-content` (per-doc), `import-arched-image` + `maintain-homepage-hero` (card art), `prune-content` (the shells a split leaves) |
